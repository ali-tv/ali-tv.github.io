...
- Tüm #EXTINF satırlarına group-title="Filmler🎭" ekler/overridelar
- Tüm #EXTINF satırlarına copyright="@ali_hsyn3" ekler/overridelar
- Çıktıyı belirtilen dosyaya yazar

Kullanım:
  python3 m3u_rewriter.py --source "https://ornek.com/playlist.m3u" --output ./ali_filmler.m3u
"""
...
try:
    ...
    print("requests kütüphanesi gerekli. Kurmak için: pip install requests")
    sys.exit(1)
...
def rewrite_extinf_line(line, group_title='Filmler🎭', copyright_tag='@ali_hsyn3'):
    """
    line örn: #EXTINF:-1 tvg-id="..." tvg-name="..." group-title="Eski",Film Adı
    Amac: attribute kısmını düzenle / ekle, son olarak ",Başlık" kısmını koru
    """
    m = ATTRS_RE.match(line)
    if not m:
        # eğer beklenen formatta değilse, en basit şekilde group-title & copyright ekle
        if line.strip().startswith('#EXTINF'):
            # eklemeden önce virgül varsa başlık kısmını koru
            if ',' in line:
...
def main():
    ...
    p.add_argument('--output', '-o', required=True, help='Çıkış dosyası (ör. ali_filmler.m3u)')
    p.add_argument('--group', '-g', default='Filmler🎭', help='group-title değeri (default: Filmler🎭)')
    p.add_argument('--copyright', '-c', default='@ali_hsyn3', help='copyright değeri (default: @ali_hsyn3)')
    p.add_argument('--timeout', type=int, default=15, help='İstek zaman aşımı saniye (default 15)')
    p.add_argument('--user-agent', default=None, help='Özel User-Agent header')
    args = p.parse_args()
